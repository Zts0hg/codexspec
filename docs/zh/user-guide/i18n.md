...
CodexSpec 将语言拆分为四个可独立配置的维度。`output` 是基础；其余三项覆盖它，并在未设置时回退到它（再到 `en`）。
...
> **注意**：`/codexspec:translate-docs` 和 `/codexspec:check-i18n-semantics` 命令是 **CodexSpec 维护者专用工具**。它们与本仓库的 MkDocs 多语言目录结构（`docs/{lang}/`）以及仅存在于本仓库的术语表 `docs/i18n/glossary.yml` 紧密耦合。它们**不会**通过 `codexspec init` 分发到用户项目中，也**不**作为面向终端用户的斜杠命令提供。
>
> 如果你正在开发 CodexSpec 本身，这两个命令位于 `.claude/commands/codexspec/`，调用方式与其他斜杠命令相同。需要翻译自己项目文档的终端用户，请直接使用 Claude 并结合自己偏好的工作流完成翻译。

CodexSpec 维护者可以使用 `/codexspec:translate-docs` 命令来翻译文档：
...
位于 `docs/i18n/glossary.yml` 的术语表确保术语一致性：
...
译文力求用你的语言自然表达，而非逐字照搬英文：

- **意思优先。** 每段文字先理解其要传达的内容，再用目标语言自己的措辞与句式重新表达。
- **英文术语更清晰时保留英文。** 没有合适母语对应、或在英文中更易识别的技术术语，保留英文而非生硬翻译。例如 "AI coding agent" 译成 "AI 编程助手" 这样的自然说法，而不是生硬直译。
...
`.github/workflows/docs-i18n.yml` 工作流是**仅供构建关卡使用**：每当 `docs/**` / `mkdocs.yml` 发生变更（推送到 `main` 以及 Pull Request）时，它会在所有语言版本上运行 `mkdocs build --strict`，以捕获构建错误、断链和格式错误的内容。它**不会**翻译任何内容。

翻译通过 `/codexspec:translate-docs`（见上文）手动生成并提交，英文源文件与所有译文在同一个经过审阅的原子提交中。CI 中的自动翻译已被有意移除 —— 此前的那个任务从未真正运行过（一个格式错误的 `if` 跳过了它），需要一个常驻的 API key，而且会把未经审阅的译文直接提交到 `main`。
