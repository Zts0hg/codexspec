...
選択した Git 変更を、マージ前の厳格な欠陥ゲートとしてレビューします。デフォルト対象は機能の完全な差分です。明示的なセレクターでコミット済み、未コミット、または単一コミットの証拠を選べますが、パスフィルターは受け付けません。
...
- デフォルトは広範な品質スコアカードではなく、変更範囲の欠陥ゲートになります。
- 位置引数のパスは無効になります。助言的なパス品質監査には `--audit` を明示してください。
...
明示的な audit ブランチでは、現在のファイル内容全体を対象に慣用的明確さ、正確性、堅牢性、アーキテクチャ、憲法整合性を評価します。このスコアカードは助言用であり、`implement-tasks` の完了条件にはなりません。
