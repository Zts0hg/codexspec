...
- Target requires `selector`, `fingerprint`, `complete_feature`, `empty`, `base_ref`, `merge_base_sha`, `commit_sha`, `parent_sha`, and `inventory_count` with known types. `empty` agrees with inventory count. Selectors `uncommitted` and `commit` cannot claim `complete_feature`; selector `commit` requires `commit_sha`, while other selectors require both commit-only SHA fields to be null. `requirements_coverage.status: complete` requires a complete-feature target. A non-empty target has at least one contract and review partition.
