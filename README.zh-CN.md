...
- **持久化需求确认** - `/specify` 在生成文档前，将确认后的讨论记录到 `requirements.md`
- **自动审查** - 生成的规格、计划与任务产物都包含内置质量检查
- **可追踪任务** - 任务分解保留需求与计划覆盖关系，仅在适用时采用测试优先
...
确认后的需求是功能范围内最高优先级的事实来源。后续产物通过来源标识保持可追踪，发生冲突时不得静默覆盖。
...
|------|----------------------|----------------------|
| **目的** | 初始需求探索与确认 | 细化确认需求或派生规格 |
| **何时使用** | 开始一个功能时 | 需求或规格需要澄清时 |
| **输出** | 创建/更新 `requirements.md` | 先更新 `requirements.md`，再同步 `spec.md` |
| **方法** | 开放式问答 | 结构化扫描（4 类别） |
| **问题数量** | 无限制 | 每次最多 5 个 |

### 核心概念：迭代质量循环

每个生成命令都包含**自动审查**。只有具备证据的缺陷才可自动修复并复审，且最多两轮；建议项单独列出，不触发自动修改。
...
- 探索您可能未考虑到的边界情况
- 请求您确认最终需求摘要
- 将确认的需求、约束、决策、排除项和开放问题持久化到 `requirements.md`
...
- 将 `requirements.md` 中已确认的条目编译为结构化规格
- 为需求添加来源引用，保持可追踪性
...
只生成与功能相关的规划内容，通过 `Covers` 关联规格需求，并验证适用的项目原则。
...
- **条件测试**：仅在计划、项目原则或任务风险要求时采用测试优先顺序
- **并行标记 `[P]`**：仅用于真正独立的任务
- **文件路径规范**：每个任务有明确的交付物
- **可追踪性**：每个任务关联其覆盖的计划项与需求
...
| `/codexspec:plan-to-tasks` | 将计划分解为可追踪、可验证的任务 ★ 自动审查 |
...
3. **基于证据的审查**：缺陷必须提供具体证据；设计建议不影响验收
4. **人工检查点**：工作流围绕验证关卡设计
