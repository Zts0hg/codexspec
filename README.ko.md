...
- [왜 CodexSpec을 선택해야 할까요?](#왜-codexspec을-선택해야-할까요)
...
| `--interaction-lang` | 상호작용 언어 (LLM 대화 + CLI 출력); `--lang`을 덮어씀 |
| `--document-lang` | 문서 언어 (생성된 spec/plan/tasks); `--lang`을 덮어씀 |
...
| `--set-interaction-lang` | 상호작용 언어 설정 |
...
CodexSpec은 언어를 개별적으로 구성 가능한 네 가지 차원으로 나눕니다. `output`이 기반이며, 나머지는 이를 덮어쓰고 설정되지 않았을 때 이 언어(그 다음 `en`)로 폴백합니다. 따라서 Claude와는 한 언어로 대화하면서 생성된 산출물이나 커밋 메시지는 다른 언어로 유지할 수 있습니다.
